...
# Map subdirectory to docs.md category header
map_subdir_to_category() {
    local subdir=$1
    case "$subdir" in
        mac) echo "### Mac" ;;
        nodejs) echo "### Node.js" ;;
        claudeCodeCli) echo "### Claude Code CLI" ;;
        *) echo "### Git" ;;
    esac
}

# Update _tabs/docs.md navigation index with a new document entry
update_nav_index() {
    local file=$1
    local title=$2
    local nav_file="$SITE_ROOT/_tabs/docs.md"

    if [ ! -f "$nav_file" ]; then
        print_error "Navigation file not found: $nav_file"
        return 1
    fi

    # Extract subdirectory from file path
    local filedir=$(dirname "$file")
    local subdir=$(basename "$filedir")

    # Map subdirectory to category
    local category=$(map_subdir_to_category "$subdir")

    # Check if entry already exists
    if grep -q "\- \[${title}\]" "$nav_file"; then
        print_info "Document already in navigation index"
        return 0
    fi

    # Construct expected URL path
    local expected_url="/docs/${subdir}/$(basename "$file" .md)/"
    local nav_entry="- [${title}](${expected_url})"

    # Find the last list item (- [xxx]) in the category section
    # and insert the new entry after it
    local found_last_item=false

    # Use awk to find the last list item line under the category and insert after it
    ...
    ' "$nav_file" > /tmp/docs_nav_new.md

    if mv /tmp/docs_nav_new.md "$nav_file"; then
        print_success "Navigation index updated: added '$title' to '$category' section"
        return 0
    else
        print_error "Failed to update navigation index"
        return 1
    fi
}
...
process_document() {
    ...
    print_info "Updating navigation index..."
    update_nav_index "$file" "$title"
    echo ""
...
usage() {
    ...
    echo "  2. Update navigation index in _tabs/docs.md"
    echo "  3. Build the Jekyll site"
    echo "  4. Verify search index includes the document"
    echo "  5. Verify document URL/route is correct"
    echo "  6. Optionally commit and push to Git"
}
